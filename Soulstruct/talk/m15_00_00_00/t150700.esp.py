...
class State_InteractionStart(State):
    ...
    def test(self):
        ...
        if GetFlagState(1600) == 0 and GetFlagState(1601) == 0 and GetOneLineHelpStatus() == 0 and HasDisableTalkPeriodElapsed() == 1 and IsTalkingToSomeoneElse() == 0 and CheckSelfDeath() == 0 and IsCharacterDisabled() == 0 and IsClientPlayer() == 0 and GetRelativeAngleBetweenPlayerAndSelf() <= 45 and GetDistanceToPlayer() <= 2:
...
class State_OpenShopReal(State):
    ...
    def enter(self):
        OpenRegularShop(1000, 1099)  # Andre's range.
