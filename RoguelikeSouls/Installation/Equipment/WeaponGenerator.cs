...
namespace RoguelikeSouls.Installation
{
    enum Upgrade
    {
        Normal = 0,  // Default. No special effects.
        Crystal = 100,  // Stronger, but less durable.
        Lightning = 200,  // Lightning-type damage. More effective against dragons.
        Refined = 300,  // Simple damage upgrade.
        Magic = 400,  // Magic-type damage.
        Enchanted = 500,  // Magic-type damage (weaker than Magic upgrade) and restores spell casts on hit.
        Divine = 600,  // Heals you slightly on hit.
        Dire = 700,  // Gets stronger as you lose more lives.
        Fire = 800,  // Fire-type damage.
        Draconic = 900,  // Lingering fire-type damage and self-damage.
    }
    ...
    class WeaponStats
    {
        ...
        public WeaponStats(
            ...
            int oneHandAnim, int twoHandAnim, int baseAttackAnim, int specialAttackAnim
        )
        {
            // Simple container for information about a vanilla weapon.
            Name = weaponName;
    ...
    class WeaponGenerator
    {
        ...
        {
            { "Dagger", (9, 11, 8, 8) },
            { "StraightSword", (12, 10, 8, 8) },
            { "Greatsword", (20, 10, 8, 8) },
            { "UltraGreatsword", (30, 10, 8, 8) },
            { "CurvedSword", (10, 12, 8, 8) },
            { "CurvedGreatsword", (18, 16, 8, 8) },
            { "ThrustingSword", (12, 14, 8, 8) },
            { "Katana", (16, 18, 8, 8) },
            { "Axe", (16, 8, 8, 8) },
            { "Greataxe", (32, 8, 8, 8) },
            { "Hammer", (12, 11, 8, 8) },
            { "GreatHammer", (30, 11, 8, 8) },
            { "Fists", (10, 10, 8, 8) },
            { "Spear", (14, 14, 8, 8) },
            { "Halberd", (16, 14, 8, 8) },
            { "Scythe", (14, 16, 8, 8) },
        ...
        public bool SkipBehaviors { get; set; } = false;
        ...
        void CreateStartingWeapons()
        {
            ...
            StartingWeaponSetup(1365000, 51000, ref behaviorVarId);  // Sunlight Talisman
            StartingWeaponSetup(1472000, 52000, ref behaviorVarId);  // Sunlight Shield
        ...
        )
        {
            ...
            Weapon modelWeapon = modelWeaponStats.GetWeaponParam(Mod.VanillaGPARAM);
            ...
            // Combat statistics are randomized mixtures of model and base weapons.
            RandomizeCombatValues(newWeapon, modelWeapon, baseWeapon);
        ...
        void RandomizeRequiredStats(Weapon weapon, string weaponSubclass, double randomRange)
        {
            // e.g. if `randomRange == 0.1`, then each stat will vary randomly (rounded) from 0.9 * base to 1.1 * base.
        ...
        void RandomizeCombatValues(Weapon weapon, Weapon modelWeapon, Weapon baseWeapon)
        {
            // Each stat here is calculated by choosing a base vs. model weight factor from a normal distribution.
            // It could lean toward the model (e.g. mean = 0.3) or toward the animation base (e.g. mean = 0.7).
            // Most lean toward the base, since animation speed is generally more relevant for balance than hitbox size.
            double w;
            w = Clamp(GetRandNormal(0.5, 0.2));
            weapon.Weight = (ushort)(w * baseWeapon.Weight + (1.0 - w) * modelWeapon.Weight);

            w = Clamp(GetRandNormal(0.7, 0.15));
            weapon.BasePhysicalDamage = (ushort)(w * baseWeapon.BasePhysicalDamage + (1.0 - w) * modelWeapon.BasePhysicalDamage);
            w = Clamp(GetRandNormal(0.7, 0.15));
            weapon.BaseStaminaDamage = (ushort)(w * baseWeapon.BaseStaminaDamage + (1.0 - w) * modelWeapon.BaseStaminaDamage);
            w = Clamp(GetRandNormal(0.7, 0.15));
            weapon.BasePoiseDamage = (ushort)(w * baseWeapon.BasePoiseDamage + (1.0 - w) * modelWeapon.BasePoiseDamage);

            w = Clamp(GetRandNormal(0.7, 0.15));
            weapon.PhysicalGuardPercentage = (ushort)(w * baseWeapon.PhysicalGuardPercentage + (1.0 - w) * modelWeapon.PhysicalGuardPercentage);
            w = Clamp(GetRandNormal(0.7, 0.15));
            weapon.MagicGuardPercentage = (ushort)(w * baseWeapon.MagicGuardPercentage + (1.0 - w) * modelWeapon.MagicGuardPercentage);
            w = Clamp(GetRandNormal(0.7, 0.15));
            weapon.FireGuardPercentage = (ushort)(w * baseWeapon.FireGuardPercentage + (1.0 - w) * modelWeapon.FireGuardPercentage);
            w = Clamp(GetRandNormal(0.7, 0.15));
            weapon.LightningGuardPercentage = (ushort)(w * baseWeapon.LightningGuardPercentage + (1.0 - w) * modelWeapon.LightningGuardPercentage);
            w = Clamp(GetRandNormal(0.7, 0.15));
            weapon.GuardStaminaDefense = (short)(w * baseWeapon.GuardStaminaDefense + (1.0 - w) * modelWeapon.GuardStaminaDefense);
            w = Clamp(GetRandNormal(0.7, 0.15));
            weapon.PoisonDamageReductionWhenGuarding = (sbyte)(w * baseWeapon.PoisonDamageReductionWhenGuarding + (1.0 - w) * modelWeapon.PoisonDamageReductionWhenGuarding);
            w = Clamp(GetRandNormal(0.7, 0.15));
            weapon.ToxicDamageReductionWhenGuarding = (sbyte)(w * baseWeapon.ToxicDamageReductionWhenGuarding + (1.0 - w) * modelWeapon.ToxicDamageReductionWhenGuarding);
            w = Clamp(GetRandNormal(0.7, 0.15));
            weapon.BleedDamageReductionWhenGuarding = (sbyte)(w * baseWeapon.BleedDamageReductionWhenGuarding + (1.0 - w) * modelWeapon.BleedDamageReductionWhenGuarding);
            w = Clamp(GetRandNormal(0.7, 0.15));
            weapon.CurseDamageReductionWhenGuarding = (sbyte)(w * baseWeapon.CurseDamageReductionWhenGuarding + (1.0 - w) * modelWeapon.CurseDamageReductionWhenGuarding);
        }

        double GetRandNormal(double mean, double std)
        {
            // Generates a random number from the (Box-Muller approximated) normal distribution with given mean and std.
        ...
        void CleanWeaponParam(Weapon weapon, string weaponSubclass, bool isStartingWeapon = false)
        {
            ...
            // These default required stats will generally be replaced afterward.
        ...
        double Clamp(double x, double min = 0.0, double max = 1.0)
        {
